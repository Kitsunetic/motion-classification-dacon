...
class TFT(nn.Module):
    def __init__(self, config):
        ...
        for i in range(self.time_varying_categoical_variables):
            emb = TimeDistributed(
                nn.Embedding(config["time_varying_embedding_vocab_sizes"][i], config["embedding_dim"]), batch_first=True
            ).to(self.device)
        ...
        self.lstm_encoder = nn.LSTM(
            input_size=self.hidden_size, hidden_size=self.hidden_size, num_layers=self.lstm_layers, dropout=config["dropout"]
        )
        ...
        self.static_enrichment = GatedResidualNetwork(
            self.hidden_size, self.hidden_size, self.hidden_size, self.dropout, config["embedding_dim"] * self.static_variables
        )
