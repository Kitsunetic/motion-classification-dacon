...
import networks
from utils import AccuracyMeter, AverageMeter, BinaryAccuracyMeter, generate_experiment_directory
...
class QDataset(TensorDataset):
    def __getitem__(self, idx):
        ...
        if len(data) != 1:
            ...
            y = torch.tensor([1 if y.item() == 26 else 0], dtype=torch.float32)
            return x, y
        else:
            # test
            return data
...
class Trainer:
    ...
    def train_loop(self, dl):
        ...
        self.loss, self.acc = AverageMeter(), BinaryAccuracyMeter()
        for x, y in dl:
            y_ = y.cuda()
            p = self.model(x.cuda())
    ...
    def valid_loop(self, dl):
        ...
        self.val_loss, self.val_acc = AverageMeter(), BinaryAccuracyMeter()
        with torch.no_grad():
            for x, y in dl:
                y_ = y.cuda()
                p = self.model(x.cuda())
